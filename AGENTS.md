...
3. 查看 log 和控制台都使用同一个串口：`COM4 115200`。也就是说，系统日志输出和 CLI 控制台收发都在 `COM4` 上，通过 `D:\serial-AI-portable\serial-ai-cli.exe` 操作。使用前先确保 `serial-ai-gui` 已启动。
4. 常用串口命令：
   ```powershell
   ...
   & 'D:\serial-AI-portable\serial-ai-cli.exe' wait --port COM4 --contains "RTOS heap free=" --timeout 10s
   & 'D:\serial-AI-portable\serial-ai-cli.exe' wait --port COM4 --regex "value=[0-9]+" --timeout 500ms --json
   ```
5. 固件 CLI 常用测试命令：
   ...
   CLI 命令需要以 CRLF 结尾；推荐测试时使用明确包含 `0D 0A` 的 hex 发送：
   ```powershell
   & 'D:\serial-AI-portable\serial-ai-cli.exe' send --port COM4 --hex "68 65 6C 70 0D 0A"       # help
   & 'D:\serial-AI-portable\serial-ai-cli.exe' send --port COM4 --hex "69 6E 66 6F 0D 0A"       # info
   & 'D:\serial-AI-portable\serial-ai-cli.exe' send --port COM4 --hex "68 65 61 70 0D 0A"       # heap
   & 'D:\serial-AI-portable\serial-ai-cli.exe' send --port COM4 --hex "75 70 74 69 6D 65 0D 0A" # uptime
   & 'D:\serial-AI-portable\serial-ai-cli.exe' send --port COM4 --hex "74 61 73 6B 73 0D 0A"    # tasks
   ```
